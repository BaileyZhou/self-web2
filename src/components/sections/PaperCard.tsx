    ...
    <Link href={`/papers/${paper.id}`} className="group block h-full">
      <Card className="h-full">
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between gap-2 mb-3">
            <div className="flex flex-wrap gap-1">
              ...
              {(paper.topics?.length ?? 0) > 2 && (
                <span className="text-xs text-rose-400 self-center">+{(paper.topics?.length ?? 0) - 2}</span>
              )}
            </div>
            <div className="flex items-center gap-2">
              {paper.status && (
                <span className={`text-xs rounded-full px-2 py-0.5 border ${statusCls}`}>
                  {paper.status}
                </span>
              )}
          ...
          <h3 className="text-base font-semibold text-slate-800 group-hover:text-rose-600 transition-colors leading-snug">
            {paper.title}
          </h3>
          ...
          <p className="text-sm text-slate-500 leading-relaxed line-clamp-3 flex-1 mt-2">
            {paper.summary}
          </p>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 text-sm text-rose-600 font-medium inline-flex items-center gap-1.5 group-hover:gap-2.5 transition-all">
            {detailLabel}
